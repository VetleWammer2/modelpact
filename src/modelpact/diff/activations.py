...
def projected_difference(
    base: Tensor, target: Tensor, *, dimensions: int = 16, seed: int = 0
) -> tuple[float, ...]:
    ...
    projection = torch.randn(
        (dimensions, flattened.numel()), generator=generator, dtype=torch.float64
    )
    projection /= max(1.0, flattened.numel() ** 0.5)
    return tuple(float(item) for item in (projection @ flattened.cpu()).tolist())


def concatenate_fingerprints(
    values: Iterable[tuple[float, ...]], *, maximum_values: int = 256
) -> tuple[float, ...]:
    output: list[float] = []
    for value in values:
        output.extend(value)
