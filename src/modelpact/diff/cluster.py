...
def witness_vector(witness: DifferenceWitness) -> NDArray[np.float64]:
    metrics = tuple(value for _, value in sorted(witness.divergence_metrics.items()))
    values = (
        metrics
        + witness.activation_fingerprint
        + witness.gradient_fingerprint
        + witness.prompt_fingerprint
    )
    if not values:
        values = (0.0,)
...
def _padded_matrix(witnesses: tuple[DifferenceWitness, ...]) -> NDArray[np.float64]:
    ...
    normalized: NDArray[np.float64] = (matrix - matrix.mean(axis=0)) / scales
    return normalized
...
def deterministic_agglomerative(
    ...
    *,
    ...
    while len(clusters) > 1:
        ...
        distance, left, right, left_index, right_index = min(
    ...
    for number, members in enumerate(sorted(clusters), start=1):
        pairwise = np.linalg.norm(
            matrix[list(members), None, :] - matrix[None, list(members), :], axis=-1
        )
        ...
        outliers = tuple(
            ordered[members[index]].witness_id
            for index, distance in enumerate(distances)
            if distance > cutoff and len(members) > 2
        )
        results.append(
            WitnessCluster(
