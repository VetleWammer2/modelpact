...
def gradient_initializer(gradient: Tensor, *, rank: int, scale: float = 1e-3, seed: int = 0) -> InitializedFactors:
    left, right = low_rank_factors(-gradient, rank=rank, seed=seed)
    root_scale = abs(scale) ** 0.5
    sign = -1.0 if scale < 0 else 1.0
    return InitializedFactors(left * root_scale, right * root_scale * sign, "contrastive_gradient", float(gradient.norm().item()))


def target_delta_initializer(base: Tensor, target: Tensor, *, rank: int, scale: float = 1.0, seed: int = 0) -> InitializedFactors:
    ...
    root_scale = abs(scale) ** 0.5
    sign = -1.0 if scale < 0 else 1.0
    return InitializedFactors(left * root_scale, right * root_scale * sign, "target_delta_signal", float(delta.norm().item()))
