...
class MutationConfig:
    ...
    distractors: tuple[str, ...] = (
        ...
        "Note: this sentence is unrelated.",
    )
...
def _append(results: list[Mutation], mutation: Mutation, *, limit: int) -> None:
    if (
        mutation.mutated != mutation.source
        and all(item.mutated != mutation.mutated for item in results)
...
def mutate_prompt(
    prompt: str, *, config: MutationConfig = DEFAULT_MUTATION_CONFIG, seed: int = 0
) -> tuple[Mutation, ...]:
    """Apply every configured operator in a stable seeded order."""

    rng = random.Random(seed)  # noqa: S311 - deterministic local mutation order
    ...
    if match:
        for entity in entities:
            _append(
                results,
                Mutation(
                    MutationOperator.ENTITY_SUBSTITUTION,
                    prompt,
                    prompt[: match.start()] + entity + prompt[match.end() :],
                ),
    ...
    if number_match:
        for number in config.numbers:
            _append(
                results,
                Mutation(
                    MutationOperator.NUMBER_SUBSTITUTION,
                    prompt,
                    prompt[: number_match.start()] + str(number) + prompt[number_match.end() :],
                ),
                limit=config.maximum_per_prompt,
            )
    for casing in (prompt.lower(), prompt.upper(), prompt.swapcase()):
        _append(
            results,
            Mutation(MutationOperator.CASING, prompt, casing),
            limit=config.maximum_per_prompt,
        )
    for punctuation in (
        prompt.rstrip(" .!?"),
        ...
        _append(
            results,
            Mutation(MutationOperator.PUNCTUATION, prompt, punctuation),
            limit=config.maximum_per_prompt,
        )
    ...
    if len(instructions) > 1:
        _append(
            results,
            Mutation(MutationOperator.INSTRUCTION_ORDER, prompt, "\n".join(reversed(instructions))),
            limit=config.maximum_per_prompt,
        )
    for wrapper in config.role_wrappers:
        _append(
            results,
            Mutation(MutationOperator.ROLE_WRAPPER, prompt, wrapper.format(prompt=prompt)),
            limit=config.maximum_per_prompt,
        )
    for distractor in config.distractors:
        _append(
            results,
            Mutation(MutationOperator.DISTRACTOR_CONTEXT, prompt, f"{distractor}\n{prompt}"),
            limit=config.maximum_per_prompt,
        )
        _append(
            results,
            Mutation(MutationOperator.IRRELEVANT_SENTENCE, prompt, f"{prompt} {distractor}"),
            limit=config.maximum_per_prompt,
        )
        ...
        if source in prompt:
            for replacement in sorted(config.synonyms[source]):
                _append(
                    results,
                    Mutation(
                        MutationOperator.SYNONYMOUS_TEMPLATE,
                        prompt,
                        prompt.replace(source, replacement),
                    ),
                    limit=config.maximum_per_prompt,
                )
    for whitespace in (re.sub(r" +", "  ", prompt), prompt.replace(" ", "\n"), f"  {prompt}  "):
        _append(
            results,
            Mutation(MutationOperator.WHITESPACE, prompt, whitespace),
            limit=config.maximum_per_prompt,
        )
    for prefix in config.completion_prefixes:
        _append(
            results,
            Mutation(MutationOperator.COMPLETION_PREFIX, prompt, prompt + "\n" + prefix),
            limit=config.maximum_per_prompt,
        )
